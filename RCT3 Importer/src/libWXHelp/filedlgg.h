...
// Modified by: Tobias Minich (2006)
// Created:     8/17/99
// Copyright:   (c) Robert Roebling
// RCS-ID:      $Id: filedlgg.h,v 1.44 2006/06/08 10:05:42 RR Exp $
...
protected:
    ...
    // Expose ignoreChanges for subclassing
    bool GetIgnoreChanges();
    bool SetIgnoreChanges(bool newignore);

    // A virtual function called whenever the filename changes
    virtual void HandleFilenameChange() {};

    // Parts of the previous Create function to ease subclassing
    bool CreatePrepare( wxWindow *parent,
                 const wxString& message = wxFileSelectorPromptStr,
                 const wxString& defaultDir = wxEmptyString,
                 const wxString& defaultFile = wxEmptyString,
                 const wxString& wildCard = wxFileSelectorDefaultWildcardStr,
                 ...
                 const wxString& name = wxFileDialogNameStr,
                 bool bypassGenericImpl = false );
    void CreateDefaultControls(wxWindow *parent, const wxString& wildCard, bool is_pda);
    void CreateFinish(bool is_pda);
