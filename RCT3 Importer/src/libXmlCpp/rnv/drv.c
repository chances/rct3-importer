...
void drv_default_verror_handler(int erno,va_list ap)
{
    ...
    else
    {
        ...
        {
        case DRV_ER_NODTL:
            err("no datatype library for URI '%s'");
            break;
        default:
            assert(0);
        }
...
static void error_handler(int erno,...)
{
    va_list ap;
    va_start(ap,erno);
    (*drv_verror_handler)(erno,ap);
    va_end(ap);
}

static void verror_handler_xsd(int erno,va_list ap)
{
    (*drv_verror_handler)(erno|ERBIT_XSD,ap);
}
...
static void accept_m(void)
{
    ...
    {
        if (drv_compact) ht_del(&ht_m,i_m);
        else return;
    }
    ht_put(&ht_m,i_m++);
...
void drv_init(void)
{
    if (!initialized)
    {
        ...
        rn_init();
        xsd_init();
        xsd_verror_handler=&verror_handler_xsd;
...
static int ncof(int nc,int uri,int name)
{
    ...
    {
    case RN_NC_QNAME:
        rn_QName(nc,uri2,name2);
        return uri2==uri&&name2==name;
    case RN_NC_NSNAME:
        rn_NsName(nc,uri2);
        return uri2==uri;
    case RN_NC_ANY_NAME:
    ...
    case RN_NC_EXCEPT:
        ...
        return ncof(nc1,uri,name)&&!ncof(nc2,uri,name);
    case RN_NC_CHOICE:
...
static int apply_after(int (*f)(int q1,int q2),int p1,int p0)
{
    ...
    case RN_P_CHOICE:
        rn_Choice(p1,p11,p12);
        return rn_choice(apply_after(f,p11,p0),apply_after(f,p12,p0));
    case RN_P_AFTER:
        rn_After(p1,p11,p12);
        return rn_after(p11,(*f)(p12,p0));
    default:
        assert(0);
    }
    return 0;
...
int drv_start_tag_open(int p,char *suri,char *sname)
{
    return start_tag_open(p,rn_newString(suri),rn_newString(sname),0);
}
int drv_start_tag_open_recover(int p,char *suri,char *sname)
{
    return start_tag_open(p,rn_newString(suri),rn_newString(sname),1);
}

static int puorg_rn(int p2,int p1)
{
    return rn_group(p1,p2);
}
...
static int attribute_open(int p,int uri,int name)
{
    ...
    newAttributeOpen(p,uri,name);
    M_SET(ret);
    accept_m();
    return ret;
}

int drv_attribute_open(int p,char *suri,char *sname)
{
    return attribute_open(p,rn_newString(suri),rn_newString(sname));
}
...
{
    return drv_end_tag(p);
}
extern int drv_attribute_close_recover(int p)
{
    return drv_end_tag_recover(p);
}
...
{
    return start_tag_close(p,0);
}
int drv_start_tag_close_recover(int p)
{
    return start_tag_close(p,1);
}
...
int drv_text(int p,char *s,int n)
{
    return textws(p,s,n);
}
int drv_text_recover(int p,char *s,int n)
{
    return p;
}
...
int drv_mixed_text(int p)
{
    return mixed_text(p);
}
int drv_mixed_text_recover(int p)
{
    return p;
}
