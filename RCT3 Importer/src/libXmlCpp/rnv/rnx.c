...
static void expected(int p,int first,int req)
{
    ...
    case RN_P_CHOICE:
        rn_Choice(p,p1,p2);
        expected(p1,first,req);
        expected(p2,first,req);
        break;
    ...
    case RN_P_GROUP:
        rn_Group(p,p1,p2);
        expected(p1,first,req);
        expected(p2,first&&rn_nullable(p1),req);
        break;
...
char *rnx_p2str(int p)
{
    ...
    case RN_P_CHOICE:
        ...
        break;
    case RN_P_INTERLEAVE:
        s=s_clone("interleave (&)");
        break;
    case RN_P_GROUP:
        s=s_clone("group (,)");
        break;
    case RN_P_ONE_OR_MORE:
        s=s_clone("one or more (+)");
        break;
    case RN_P_LIST:
        s=s_clone("list");
        break;
