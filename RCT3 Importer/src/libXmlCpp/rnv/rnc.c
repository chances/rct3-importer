...
void rnc_default_verror_handler(int er_no,va_list ap)
{
    ...
    case RNC_ER_XESC:
        ...
        break;
    case RNC_ER_LEXP:
        err("lexical error: '%c' expected");
        break;
    case RNC_ER_LLIT:
        err("lexical error: unterminated literal");
        break;
    case RNC_ER_LILL:
        err("lexical error: illegal character \\x{%x}");
        break;
    case RNC_ER_SEXP:
        err("syntax error: %s expected, %s found");
        break;
    case RNC_ER_SILL:
        err("syntax error: %s unexpected ");
        break;
    case RNC_ER_NOTGR:
        err("included schema is not a grammar");
        break;
    case RNC_ER_EXT:
        err("cannot open external grammar '%s'");
        break;
    case RNC_ER_DUPNS:
        err("duplicate namespace prefix '%s'");
        break;
    case RNC_ER_DUPDT:
        err("duplicate datatype prefix '%s'");
        break;
    case RNC_ER_DFLTNS:
        warn("overriding default namespace prefix '%s'");
        break;
    case RNC_ER_DFLTDT:
        warn("overriding default datatype prefix '%s'");
        break;
    case RNC_ER_NONS:
        err("undeclared namespace prefix '%s'");
        break;
    case RNC_ER_NODT:
        err("undeclared datatype prefix '%s'");
        break;
    case RNC_ER_NCEX:
        err("first argument for '-' is not '*' or 'prefix:*'");
        break;
    case RNC_ER_2HEADS:
        err("repeated define or start");
        break;
    case RNC_ER_COMBINE:
        err("conflicting combine methods in define or start");
        break;
    case RNC_ER_OVRIDE:
        err("'%s' overrides nothing");
        break;
    case RNC_ER_EXPT:
        err("first argument for '-' is not data");
        break;
    case RNC_ER_INCONT:
        err("include inside include");
        break;
    case RNC_ER_NOSTART:
        err("missing start");
        break;
    case RNC_ER_UNDEF:
        err("undefined reference to '%s'");
        break;
    default:
        assert(0);
    }
...
/* XMLCPP start */
xmlParserInputBufferPtr rnc_bind_internal(struct rnc_source *sp,char *fn,xmlParserInputBufferPtr fd)
{
    rnc_source_init(sp,fn);
    if ((sp->fd=fd))
    {
        sp->buf=(char*)m_alloc(BUFSIZE,sizeof(char));
        sp->flags=SRC_FREE;
        sp->n=sp->i=0;
        sp->complete=0;
        rnc_read(sp);
        sp->i=u_bom(sp->buf,sp->n);
    }
    return sp->fd;
}
...
xmlParserInputBufferPtr rnc_bind(struct rnc_source *sp,char *fn,int fd)
{
    xmlParserInputBufferPtr _fd = rnc_bind_internal(sp,fn,xmlParserInputBufferCreateFd(fd, XML_CHAR_ENCODING_UTF8));
    if (!_fd)
        error(1,sp,RNC_ER_IO,sp->fn,-1,-1,strerror(ENOENT));
    sp->flags|=SRC_CLOSE;
    return _fd;
}

xmlParserInputBufferPtr rnc_open(struct rnc_source *sp,char *fn)
{
    xmlParserInputBufferPtr fd = rnc_bind_internal(sp,fn,xmlParserInputBufferCreateFilename(fn, XML_CHAR_ENCODING_UTF8));
    if (!fd)
        error(1,sp,RNC_ER_IO,sp->fn,-1,-1,strerror(ENOENT));
    sp->flags|=SRC_CLOSE;
    return fd;
...
int rnc_close(struct rnc_source *sp)
{
    ...
    if (sp->flags&SRC_CLOSE)
    {
        ...
        if (sp->fd)
        {
            ret=sp->fd->closecallback(sp->fd->context);
            xmlFree(sp->fd);
            sp->fd=0;
        }
    }
...
static int rnc_read(struct rnc_source *sp)
{
    int ni,i;
    sp->n-=sp->i;
    for (i=0;i!=sp->n;++i) sp->buf[i]=sp->buf[i+sp->i];
    sp->i=0;
    for (;;)
    {
        /* XMLCPP start */
        if (sp->fd) {
            ni = sp->fd->readcallback(sp->fd->context, sp->buf+sp->n, BUFSIZE-sp->n);
        } else {
            ni = 0;
        }
        /* XMLCPP end */
        ...
        /* XMLCPP start */
            if (sp->fd)
            {
                sp->fd->closecallback(sp->fd->context);
                xmlFree(sp->fd);
                sp->fd=0;
            }
        /* XMLCPP end */
...
void rnc_init(void)
{
    if (!initialized)
    {
        ...
        rn_init();
        len_p=LEN_P;
        path=(char*)m_alloc(len_p,sizeof(char));
        /* initialize scopes */
        sc_init(&nss);
        sc_init(&dts);
        sc_init(&defs);
...
static char *sym2str(int sym)
{
    ...
    case SYM_OPTIONAL:
        return "\"?\"";
    case SYM_ZERO_OR_MORE /*SYM_ANY_NAME*/:
        return "\"*\"";
    case SYM_ONE_OR_MORE:
        return "\"+\"";
    case SYM_LPAR:
        return "\"(\"";
    case SYM_RPAR:
...
static void advance(struct rnc_source *sp)
{
    ...
    for (;;)
    {
        NXT(sp).line=sp->line;
        NXT(sp).col=sp->col;
        if (newline(sp->v)||whitespace(sp->v))
        {
            getv(sp);
            continue;
        }
        ...
        case '|':
            ...
            {
                getv(sp);
                NXT(sp).sym=SYM_ASGN_CHOICE;
                return;
            }
            NXT(sp).sym=SYM_CHOICE;
            return;
        case '&':
            getv(sp);
            if (sp->v=='=')
            {
                getv(sp);
                NXT(sp).sym=SYM_ASGN_ILEAVE;
            }
            else NXT(sp).sym=SYM_ILEAVE;
            return;
        case '?':
            getv(sp);
            NXT(sp).sym=SYM_OPTIONAL;
            return;
        case '*':
            getv(sp);
            NXT(sp).sym=SYM_ZERO_OR_MORE;
            return; /* SYM_ANY_NAME */
        case '+':
            getv(sp);
            NXT(sp).sym=SYM_ONE_OR_MORE;
            return;
        case '-':
            getv(sp);
            NXT(sp).sym=SYM_EXCEPT;
            return;
        case '~':
            getv(sp);
            NXT(sp).sym=SYM_CONCAT;
            return;
        case '(':
            getv(sp);
            NXT(sp).sym=SYM_LPAR;
        ...
        case '>':
            ...
            getv(sp);
            NXT(sp).sym=SYM_FOLLOW_ANNOTATION;
            return;
        case '"':
        case '\'':
        {
    ...
    if (CUR(sp).sym==SYM_LSQU)
    {
        ...
        for (;;)
        {
            ...
            case SYM_LSQU:
                ...
                break;
            case SYM_IDENT:
            case SYM_QNAME: /* keywords are in the default: clause */
            case SYM_ASGN:
            case SYM_LITERAL:
            case SYM_CONCAT:
                advance(sp);
...
static void addns(struct rnc_source *sp,int pfx,int url)
{
    ...
    if ((i=sc_find(&nss,pfx)))
    {
        ...
        {
            nss.tab[i][1]=url;
            nss.tab[i][2]&=~(PFX_INHERITED|PFX_DEFAULT);
        }
...
static int decl(struct rnc_source *sp)
{
    ...
    case SYM_NAMESPACE:
        getsym(sp);
        if (chkwd(sp)) pfx=rn_newString(CUR(sp).s);
        getsym(sp);
        chk_get(sp,SYM_ASGN);
        uri=nsuri(sp);
    ...
    case SYM_DATATYPES:
        ...
        getsym(sp);
        chk_get(sp,SYM_ASGN);
        if (chksym(sp,SYM_LITERAL)) uri=rn_newString(CUR(sp).s);
        getsym(sp);
...
static int element(struct rnc_source *sp)
{
    ...
    nc=nameclass(sp);
    chk_get(sp,SYM_LCUR);
    p=rn_newElement(nc,pattern(sp));
    chk_skip_get(sp,SYM_RCUR);
    return p;
}

static int rnc_local_attribute(struct rnc_source *sp)
{
    ...
    nss.tab[i][1]=nsuri;
    chk_get(sp,SYM_LCUR);
    p=rn_newAttribute(nc,pattern(sp));
    chk_skip_get(sp,SYM_RCUR);
    return p;
}
...
static int relpath(struct rnc_source *sp)
{
    ...
    if ((ret=chksym(sp,SYM_LITERAL)))
    {
        ...
        {
            m_free(path);
            path=(char*)m_alloc(len_p=len,sizeof(char));
        }
        strcpy(path,CUR(sp).s);
        s_abspath(path,sp->fn);
    }
    getsym(sp);
    return ret;
}
...
static void add_well_known_nss(int dflt)
{
    ...
    sc_add(&nss,0,dflt,PFX_INHERITED);
    sc_add(&nss,-1,dflt,PFX_INHERITED);
}
...
static int primary(struct rnc_source *sp)
{
    ...
    case SYM_TOKEN:
    case SYM_QNAME:
        return NXT(sp).sym==SYM_LITERAL?value(sp):data(sp);
    case SYM_LITERAL:
        return value(sp);
...
static int unary(struct rnc_source *sp)
{
    ...
    case SYM_OPTIONAL:
        getsym(sp);
        p=rn_choice(p,rn_empty);
        break;
    case SYM_ZERO_OR_MORE:
        getsym(sp);
        p=rn_choice(rn_one_or_more(p),rn_empty);
        break;
    case SYM_ONE_OR_MORE:
        getsym(sp);
        p=rn_one_or_more(p);
...
static void define(struct rnc_source *sp,int name)
{
    ...
    case SYM_ASGN_ILEAVE:
        ...
        break;
    default:
        error(0,sp,RNC_ER_SEXP,sp->fn,CUR(sp).line,CUR(sp).col,"assign method",sym2str(CUR(sp).sym));
    }
    getsym(sp);
    pat=pattern(sp);
...
static int grammarContent(struct rnc_source *sp)
{
    ...
    case SYM_IDENT:
        ...
        case SYM_LSQU:
            getsym(sp);
            return 1; /* skip grammar annotation */
        case SYM_ASGN:
        case SYM_ASGN_CHOICE:
        case SYM_ASGN_ILEAVE:
        {
            int name=rn_newString(CUR(sp).s);
            getsym(sp);
            define(sp,name);
            return 1;
        }
...
int rnc_parse(struct rnc_source *sp)
{
    ...
    sc_open(&nss);
    add_well_known_nss(0);
    open_scope(sp);

    start=topLevel(sp);
    if (start!=-1) sc_add(&defs,0,start,0);
